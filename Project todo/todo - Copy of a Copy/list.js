...
function addTask(){
	if($('input#addTask').val() !== ""){
		var LiID = $('.chosen').attr('id'), task = $('input#addTask').val(), completed = false;
		...
		$.ajax({
			...
			cache: false,
			complete: function(){
				$('.popupList ol').before("<li class='"+$('#taskID').val()+" li' data-index='' data-position=''><a class='handle'>:::</a><input class='listCheck' onclick='doneLi(this)' type='checkbox'><a onclick='deleteTask(this); SQLremoveLi(this)' class='del'>+</a><div onkeyup='editTask(this)' spellcheck='false' class='editable' aria-multiline='true' contenteditable>"+task+"</div>"+"</li>");
				$('input#addTask').val('');
			}
		}).done(function(response, statusT, jqXHR){
			var id = response;
...
function addToPop(elm){
	$('.popupList').append("<div class='addBox'><input type='text' placeholder='Add a task' id='addTask'><input type='button' onclick='addTask()' value='+'></div>");
...
function fetchTasks(){
	$.ajax({
		...
		for(i=0;i<x.length;i++){
			...
			if(completed == true){
				$('#'+LiID).children('ol').append("<li class='"+TID+" li done' data-index='' data-position='"+position+"'><a class='handle'>:::</a><input class='listCheck' onclick='doneLi(this)' type='checkbox' checked><a onclick='deleteTask(this); SQLremoveLi(this)' class='del'>+</a><div onkeyup='editTask(this)' spellcheck='false' class='editable' aria-multiline='true' contenteditable>"+task+"</div>"+"</li>");
				$('input#addTask').val('');
			}else{
				$('#'+LiID).children('ol').before("<li class='"+TID+" li' data-index='' data-position='"+position+"'><a class='handle'>:::</a><input class='listCheck' onclick='doneLi(this)' type='checkbox'><a onclick='deleteTask(this); SQLremoveLi(this)' class='del'>+</a><div onkeyup='editTask(this)' spellcheck='false' class='editable' aria-multiline='true' contenteditable>"+task+"</div>"+"</li>");
				$('input#addTask').val('');
			}
		}
	});
